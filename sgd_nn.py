...
def sgd(nn: NN, X_train, X_test, W, C_train, C_test, batch_size, learning_rate, iter_num, epsilon = 0.1):
    costs = []
    accuracy_train = []
    accuracy_test = []
    for iter in range(iter_num):
        ...
        for i in range(int(m / batch_size)):
            ...
            nn.update_thetas(list(reversed(weight_grads)), list(reversed(bias_grads)), learning_rate)
            costs.append(cost)
...
if __name__ == '__main__':
    ...
    learning_rate = 1
    batch_size = 60
    nn = NN([2, 5,6 ,8,10,5], np.tanh, None, tan_h_gradient, None)
    trainSetX, trainSetY, testSetX, testSetY, theta, bias = extract_sgd_data()
    W_train, costs_train, accuracy_train, accuracy_test = sgd(nn, trainSetX, testSetX, theta, trainSetY, testSetY, batch_size, learning_rate, iter_num)
